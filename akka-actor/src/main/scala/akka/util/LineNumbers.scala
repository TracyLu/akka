...
object LineNumbers {
  ...
  private def getStreamForClass(c: Class[_]): Option[(InputStream, None.type)] = {
    ...
    dbg(s"    resource '$resource' resolved to stream $r")
    Option(r).map(_ -> None)
  }
...
//            .map(_ -> Some(serialized.getImplMethodName))
  ...
  private def readMethod(d: DataInputStream, code: Int, lnt: Int, filter: Option[String])(implicit c: Constants): Option[(Int, Int)] = {
    skip(d, 2) // access flags
    val name = d.readUnsignedShort() // name
    skip(d, 2) // signature
    dbg(s"  ${c(name)}")
    val attributes =
      for (_ ← 1 to d.readUnsignedShort()) yield {
        ...
        val length = d.readInt()
        if (tag != code || (filter.isDefined && c(name) != filter.get)) {
          skip(d, length)
          None
        } else {
